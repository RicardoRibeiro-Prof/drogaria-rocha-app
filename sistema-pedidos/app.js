...
async function saveOrder(event){event.preventDefault();const button=$('saveButton');$('formError').textContent='';setBusy(button,true,'Salvar anotação');const payload={produto:$('productName').value.trim(),quantidade:Number($('quantity').value),prioridade:$('priority').value,fornecedor:$('supplier').value.trim()||null,observacao:$('notes').value.trim()||null,status:'anotado',criado_por:employeeName()};try{const res=await fetch(API,{method:'POST',headers:headers({Prefer:'return=representation'}),body:JSON.stringify(payload)});if(!res.ok)throw new Error(await res.text());$('orderModal').close();$('orderForm').reset();$('quantity').value=1;$('priority').value='normal';toast('Produto anotado com sucesso.');await loadOrders();}catch{$('formError').textContent='Não foi possível salvar. Tente novamente.';}finally{setBusy(button,false,'Salvar anotação');}}
