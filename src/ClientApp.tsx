...
function Offers({products,loading,add,openProduct}:any){
  return <ScrollView contentContainerStyle={s.page} showsVerticalScrollIndicator={false}>
    ...
    {loading?<ActivityIndicator color={C.orange}/>:products.length?<View style={s.grid}>{products.map((p:AppProduct)=><ProductCard key={p.id} p={p} add={()=>add(p)} open={()=>openProduct(p)}/>)}</View>:<View style={s.emptyCatalog}><Ionicons name="pricetags-outline" size={46} color={C.orange}/><Text style={s.emptyTitle}>Nenhuma oferta ativa agora</Text><Text style={s.muted}>Assim que uma oferta for ativada no gerenciador, ela aparecerá automaticamente aqui.</Text></View>}
  </ScrollView>;
}
...
const s=StyleSheet.create({
  ...
  detailBottomBar:{minHeight:78,backgroundColor:C.white,borderTopWidth:1,borderTopColor:C.border,paddingHorizontal:18,paddingVertical:10,flexDirection:'row',alignItems:'center',gap:14},detailBottomLabel:{fontSize:10,color:C.muted,fontWeight:'800'},detailBottomOldPrice:{fontSize:10,color:C.muted,textDecorationLine:'line-through',marginTop:1},detailBottomPrice:{fontSize:25,fontWeight:'900',color:C.black,marginTop:2},detailBuyButton:{minWidth:158,height:54,borderRadius:14,backgroundColor:C.orange,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,paddingHorizontal:17},detailBuyText:{color:C.white,fontSize:15,fontWeight:'900'},
